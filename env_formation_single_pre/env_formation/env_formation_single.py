...
class CustomEnv:
    ...
    def observe(self):
        ...
        for i in range (self.follower_uav_num):
            ...
            # for obs_id, obs in self.obstacles.items():
                ...
                # obs_distance_angle.extend([obs_distance, obs_angle])
                if _obs_distance <= self.obs_delta*2:
                    obs_num_follower += 1
                    ...
                    vo_flag, min_exp_time, min_dis = self.rvo_inter.config_vo_reward(robot_state=robot_state,
                                                                                                                                                            ...
                                                                                                                                                            action=action)
                    # print("vo_flag :" ,vo_flag)
                    px = round((obs.pos_x - self.follower_uavs[f"follower_{i}"].pos[0]) / (self.obs_delta * 2), 5)
                    py = round((obs.pos_y - self.follower_uavs[f"follower_{i}"].pos[1]) / (self.obs_delta * 2), 5)
                    vx = 0
                    vy = 0
                    _obs_distance_ = round(_obs_distance / (self.obs_delta * 2), 5)
            ...
            # 自身速度观测
            self_pos_2 = [
                round(self.follower_uavs[f"follower_{i}"].vel[0] / 2.5, 5),
            ...
            for j in range (self.follower_uav_num):
                if j != i :
                    ...
                    follow_vo_flag, min_exp_time, min_dis = self.rvo_inter.config_vo_reward(robot_state=robot_state,
                                                                                                                                                                            ...
                                                                                                                                                                            action=action)
                    px = round(
                        (self.follower_uavs[f"follower_{j}"].pos[0] - self.follower_uavs[f"follower_{i}"].pos[0])
                        / (np.linalg.norm(np.array(self.formation_pos[0])) * 2),
                        5,
                    )
                    py = round(
                        (self.follower_uavs[f"follower_{j}"].pos[1] - self.follower_uavs[f"follower_{i}"].pos[1])
                        / (np.linalg.norm(np.array(self.formation_pos[0])) * 2),
                        5,
                    )
                    vx = round(self.follower_uavs[f"follower_{j}"].vel[0] / 2.5, 5)
                    vy = round(self.follower_uavs[f"follower_{j}"].vel[1] / 2.5, 5)

                    follower_j = [px, py, vx, vy, follow_vo_flag]
                    follower_pos_.extend(follower_j)
                    # else:
                    #     follower_pos_.extend([0, 0, 0, 0, False])
            ...
            # print("UAV_", i, "  target_pos : ", target_pos_2)
            # print("UAV_", i, "  side_pos : ", side_pos_2 )
            # print( "UAV_", i,  "  obs_pos : ", obs_pos_vel_2)
            # print( "UAV_", i,  "  self_pos : ", self_pos_2)
            # print( "UAV_", i,  "  leader_vel  : ", leader_vel )
            # print("UAV_", i,  "  follower_pos_ :", follower_pos_)
           
            # print("=================")
        return leader_observation
    
    def step(self, leader_action, follower_actions, last_distance, last_obs_distance, last_follower_obs_distance, last_follower_goal_distance):
        """
        所有智能体前进一步，并更新观测 
        ...
        follower_reward = self._calculate_follower_reward(follower_actions, last_obs_distances=last_follower_obs_distance, last_follower_goal_distance = last_follower_goal_distance)
    ...
    def _apply_follower_actions(self, follower_actions):
        for i in range (self.follower_uav_num):
            self.follower_uavs[f"follower_{i}"].vel[0] = follower_actions[2*i] *2.5
            self.follower_uavs[f"follower_{i}"].vel[1] = follower_actions[2*i + 1] *2.5
            ...
            if np.linalg.norm(np.array(self.leader_agent.pos) + np.array(self.formation_pos[i]) - self.follower_uavs[f"follower_{i}"].pos) >= np.linalg.norm(np.array(self.formation_pos[0])) * 5:
                self.follower_uavs[f"follower_{i}"].formation_done =True
                flag = True

            # if flag  or self.follower_uavs[f"follower_{i}"].target:#TODO
            # if flag  or self.leader_agent.done :#TODO
            if flag  or self.leader_agent.target :#TODO
    ...
    def _apply_leader_action(self, action):
        ...
        follower_flag = True
        for i in range (self.follower_uav_num):
            if np.linalg.norm(np.array(self.leader_agent.pos) + np.array(self.formation_pos[i]) - np.array(self.follower_uavs[f"follower_{i}"].pos)) > 2.0:
                follower_flag = False

        target_dis = sqrt((x - self.leader_target_pos[0])**2 + (y - self.leader_target_pos[1])**2)
        # if target_dis < self.target_radius:  # 到达目标点 TODO
        if target_dis < self.target_radius and follower_flag == True:  # 到达目标点 TODO
...
# ======计算领航者的奖励====================
    ...
    def _leader_formation_reward(self):
        ...
        return reward
# ======计算跟随者的奖励===================
    def _calculate_follower_reward(self,follower_actions, last_obs_distances, last_follower_goal_distance):
        follower_rewards = []
        # print(follower_actions)
        for i in range (self.follower_uav_num):
            follower_action = follower_actions[2*i: 2*i+2]
            # print(follower_action)
            last_obs_distance = last_obs_distances[i]
            last_follower_goal_dis = last_follower_goal_distance[i]
            ...
            target_reward = self._follower_target_reward(i, last_follower_goal_dis)
            ...
            # r = round(target_reward + obs_collision_reward + side_reward + uav_collision_reward , 5)
            # print("uav_",i ,"r :", r, " target_reward : ", target_reward,  " side_reward : ", side_reward, " obs_reward : ", \
        ...
        # print("follower_reward_list : ", follower_rewards)
    ...
    def _follower_target_reward(self, uav_id, last_follower_goal_dis):
        ...
        if  self.follower_uavs[f"follower_{uav_id}"].formation_done:
            ...
            if dis >= np.linalg.norm(np.array(self.formation_pos[0]))*4:
                return round(-20, 5)
            # return  round(- dis *20, 5)
            return  round(20/dis, 5)
    
    def _follower_obs_collision_reward(self, uav_id, last_obs_distance):
        # if self.follower_uavs[f"follower_{uav_id}"].done and \
        # np.linalg.norm(np.array(self.leader_agent.pos) + np.array(self.formation_pos[uav_id]) - np.array(self.follower_uavs[f"follower_{uav_id}"].pos)) > 1.0:
        if self.follower_uavs[f"follower_{uav_id}"].obs_done:
            return -200
        ...
        for obs_id, obs in self.obstacles.items():
            ...
            if dis <= self.obs_delta*1.5 :     
                ...
                leader_reward =  - np.linalg.norm(np.array(self.leader_agent.pos) - np.array(self.follower_uavs[f"follower_{uav_id}"].pos))
                
                # print(f"uav_{uav_id} obs dis = ",dis, " last obs dis = ",last_obs_distance[obs_id])
                # print("min exp time :", min_exp_time)
                ...
                # print("uav", uav_id, " obs ",obs_id, " reward = ", round( (-1/dis + 1e-1) * delta, 5))

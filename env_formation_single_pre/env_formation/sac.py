...
class SAC:
    ...
    def update(self, transition_dict):
        ...
        idxs = transition_dict['idxs']

        # Critic 更新
        td_errors = self.update_critics(states, actions, rewards, next_states, dones, is_weights)
    ...
    def update_critics(self, states, actions, rewards, next_states, dones, is_weights):
        """更新 Critic 网络"""
        td_target = self.calc_target(rewards, next_states, dones)
        td_error1 = self.critic_1(states, actions) - td_target.detach()
        td_error2 = self.critic_2(states, actions) - td_target.detach()

        # 使用 is_weights 加权
        critic_1_loss = torch.mean(is_weights * td_error1 ** 2)
        critic_2_loss = torch.mean(is_weights * td_error2 ** 2)
        ...
        # 返回 TD 误差，用于更新 Replay Buffer 中的优先级
        td_errors = td_error1.abs().detach() + td_error2.abs().detach()
        return td_errors

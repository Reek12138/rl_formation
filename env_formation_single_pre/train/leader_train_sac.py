...
for episode_i in range(NUM_EPISODE):
    ...
    for step_i in range(NUM_STEP):
        ...
        follower_R_ = sum(follower_reward)/ env.follower_uav_num
        ...
        if follower_R_ >follower_highest_step_reward:
            follower_highest_step_reward = follower_R_
        follower_episode_reward = follower_episode_reward * 0.9 + follower_R_
...
#====== 网络更新========================================================================================
        ...
        batch_flag = current_memo_size >= BATCH_SIZE * 500
        if(total_step +1)% TARGET_UPDATE_INTERVAL == 0 and batch_flag == True and episode_i <= 3000:
            if env.leader_agent.replay_buffer.size() >= BATCH_SIZE:
                ...
                fs, fa, fr, fns, fd, idxs, is_weights = env.SAC.replay_buffer.sample(batch_size=BATCH_SIZE)
        ...
        if follower_done :
            ...
            for i in range(env.follower_uav_num):
                ...
                if env.follower_uavs[f"follower_{i}"].formation_done == True:
                    print(f"\033[35mxxxxxxxxxxxxxxxxxx  FOLLOWER FORMATION DONE xxxxxxxxxxxxxxxxxxx step{step_i} reward : {follower_episode_reward:.2f}\033[0m")
                    follower_break_flag = True
                    break
...
#====== 每个episode结束后更新一次=====================================================================
    if env.leader_agent.replay_buffer.size() >= BATCH_SIZE*500:
        s, a, r, ns, d = env.leader_agent.replay_buffer.sample(batch_size=BATCH_SIZE)
        transition_dict = {'states': s,
                        'actions': a,
                        'rewards': r,
    ...
    # if episode_i > NUM_EPISODE/3 and episode_i % RENDER_FREQUENCE == 0:
        ...
        num_follower_obs_collision = 0
        num_follower_uav_collision = 0
        num_follower_side_collision = 0
        num_follower_formation_done = 0
        print("======================验证中=========================")
                ...
                if follower_done :
                    ...
                    for i in range(env.follower_uav_num):
                        if env.follower_uavs[f"follower_{i}"].obs_done == True:
                            print(f"\033[93mxxxxxxxxxxxxxxxxxx  FOLLOWER OBSTICLE COLLISION xxxxxxxxxxxxxxxxxxx step{step_i} reward : {follower_episode_reward:.2f}\033[0m")
                            follower_break_flag = True
                            num_follower_obs_collision +=1
                            break
                        if env.follower_uavs[f"follower_{i}"].side_done == True:
                            print(f"\033[91mxxxxxxxxxxxxxxxxxx  FOLLOWER SIDE COLLISION xxxxxxxxxxxxxxxxxxx step{step_i} reward : {follower_episode_reward:.2f}\033[0m")
                            follower_break_flag = True
                            num_follower_side_collision +=1
                            break
                        if env.follower_uavs[f"follower_{i}"].uav_done == True:
                            print(f"\033[34mxxxxxxxxxxxxxxxxxx  FOLLOWER UAV COLLISION xxxxxxxxxxxxxxxxxxx step{step_i} reward : {follower_episode_reward:.2f}\033[0m")
                            follower_break_flag = True
                            num_follower_uav_collision +=1
                            break
                        if env.follower_uavs[f"follower_{i}"].formation_done == True:
                            print(f"\033[35mxxxxxxxxxxxxxxxxxx  FOLLOWER FORMATION DONE xxxxxxxxxxxxxxxxxxx step{step_i} reward : {follower_episode_reward:.2f}\033[0m")
                            follower_break_flag = True
                            num_follower_formation_done +=1
                            break
                    if follower_break_flag == True:
        ...
        print(f"\033[95follower collision : {num_follower_collision}, follower collision side : {num_follower_side_collision}, follower obs collision : {num_follower_obs_collision}, follower uav collosion : {num_follower_uav_collision}, follower_formation_done : {num_follower_formation_done}\033[0m")

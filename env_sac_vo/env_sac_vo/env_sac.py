...
class CustomEnv:
    ...
    def step(self, action, num_step,last_distance, last_obs_distacnce):
        ...
        # print("Observation1 shape:", observation.shape)
    ...
    def _apply_leader_action(self, agent, action, target):
        ...
        steer_angle = action[1]  * (np.pi/4)# 转向角
        ...
        # 更新航向角（yaw angle）
        new_orientation = agent.orientation + (linear_vel / L) * tan(steer_angle) * dt
        ...
        # 检测是否发生碰撞，并更新状态
        if not self._check_obs_collision(agent, new_pos) and not flag and not agent.target:
            agent.set_position(x, y)  # 更新位置
            agent.orientation = new_orientation  # 更新航向角
            agent.vel = linear_vel  # 更新速度
        else:
            agent.done = True  # 标记为完成
    ...
    def observe_leader(self, agent, target, action, last_distance, t):
        ...
        last_action = list(self.last_action[:2])  # 转换为列表
    ...
    def _caculate_velocity_reward(self, agent, action, vo_flag, last_action):
        if vo_flag:
            ...
            return (action[0] + 1) *5 - abs(action[1]) *20  - abs(action[1] - last_action[1]) *30
        else:
            # return ((action[0] + 1) ) *5 + (abs(action[1])/2)*3
            # return (action[0] + 1) *5 - abs(action[1]) *2.5  - abs(action[0] - last_action[0])*5
            return ((action[0] + 1) ) *5 - abs(action[1]) *20 - abs(action[1] - last_action[1])*5

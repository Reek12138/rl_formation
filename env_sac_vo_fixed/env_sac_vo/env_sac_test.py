...
class CustomEnv:
        ...
        for i, obs in enumerate(random_obstacles):
            ...
            while not is_position_valid:
                ...
                for j in range(i):
                    ...
                    if dis < 2 * self.obs_radius + self.agent_radius + self.safe_theta *0.5:
                        is_position_valid = False
                        break
                ...
                for fixed_obs in fixed_obstacles:
                    ...
                    if dis_fixed < 2 * self.obs_radius + self.agent_radius + self.safe_theta * 0.5:
                        is_position_valid = False
                        break
    ...
    def reset(self):
        ...
        self.leader_agent.set_position(np.random.rand() * 15, np.random.rand() * 15)
        ...
        while  np.linalg.norm(np.array(self.leader_target_pos) - np.array(self.leader_agent.pos)) < self.agent_radius + self.target_radius + self.safe_theta*4 or flag1:
            self.leader_agent.set_position(self.width*0.10+np.random.rand()*self.width*0.8, self.height*0.10+np.random.rand()*self.height*0.8)
            flag1 = self._check_obs_agent(self.leader_agent)

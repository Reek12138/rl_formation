...
class CustomEnv:
    ...
    def __init__(self, width=100, height=100, num_obstacles=15, agent_radius=1, obs_radius = 2.25,safe_theta = 8,target_radius = 4,
        ...
        self.last_action = [0 for _ in range(8)]
        self.last_traj = [[0,0,0,0] for _ in range(4)]

        

        # 生成circle_agent实例列表
        self.leader_agent = circle_agent(self, pos=[25, 25], vel=[0,0], orientation=0, memo_size=memo_size,
                                         state_dim=11 + 8 + self.num_obstacles * 5,
                                         action_dim=2 *4,
                                         alpha=1e-4,
                                         beta=1e-4,
                                         hidden_dim=512,
        ...
        for i in range(len(self.fix_position), self.num_obstacles):
            self.obstacles[f"obstacle_{i}"] = obstacle(
                radius=self.obs_radius,
                pos_x=np.random.rand() * self.width * 0.6 + self.width * 0.20,
                pos_y=np.random.rand() * self.height * 0.6 + self.height * 0.20,
                safe_theta=self.safe_theta
            )
        ...
        for i, obs in enumerate(random_obstacles):
            ...
            while not is_position_valid:
                ...
                for j in range(i):
                    ...
                    if dis < 2 * self.obs_radius + self.agent_radius + self.safe_theta*0.8:
                        is_position_valid = False
                        break
                ...
                for fixed_obs in fixed_obstacles:
                    ...
                    if dis_fixed < 2 * self.obs_radius + self.agent_radius + self.safe_theta*0.8:
                        is_position_valid = False
                        break

                # 检查与目标位置的距离
                dis2 = np.linalg.norm(np.array(self.leader_target_pos) - self.obstacles[key].position())
                if dis2 < self.obs_radius + self.target_radius + self.agent_radius + self.safe_theta*0.8:
                    is_position_valid = False

                # 如果位置无效，则重新生成随机位置
                if not is_position_valid:
                    self.obstacles[key].pos_x = np.random.rand() * self.width * 0.6 + self.width * 0.2
                    self.obstacles[key].pos_y = np.random.rand() * self.height * 0.6 + self.height * 0.2
    ...
    def reset(self):
        ...
        # self.leader_target_pos = [self.width*0.05+np.random.rand()*self.width*0.9, self.height*0.05+np.random.rand()*self.height*0.9]
        
        # self.leader_target_pos = [84 + np.random.rand()*6, 84 + np.random.rand()*6]
        ...
        # self.leader_agent.set_position(self.width*0.05+np.random.rand()*self.width*0.9, self.height*0.05+np.random.rand()*self.height*0.9)
        
        # self.leader_agent.set_position(10 +np.random.rand() * 25, 10 + np.random.rand() * 25)
        ...
        self.last_action = [0 for _ in range(8)]
        self.last_traj = [[self.leader_agent.pos[0], self.leader_agent.pos[1], 0, 0] for _ in range(4)]
    ...
    def step(self, action, num_step,last_distance, last_obs_distacnce):
        ...
        traj = self._apply_leader_action(self.leader_agent,action, self.leader_target_pos)
        
        observation = self.observe_leader(self.leader_agent, self.leader_target_pos, action, last_distance, num_step)
        reward = self._calculate_leader_reward( formation_target=self.leader_target_pos, action = action, t=num_step,
                                                last_distance=last_distance, last_obs_distance=last_obs_distacnce, traj = traj, last_traj = self.last_traj)
    ...
    def _apply_leader_action(self, agent, action, target):#TODO
        ...
        # 计算未来轨迹
        traj0 = [x, y, linear_vel, new_orientation]
        traj = [traj0]
        # 计算接下来的三个轨迹点
        for i in range(1, 4):
            linear_vel = action[2 * i] + 1
            angular_vel = action[2 * i + 1] * (np.pi / 2)
            new_orientation = new_orientation + angular_vel * self.display_time
            new_orientation = new_orientation % (2 * np.pi)
            ...
            # 将新计算的轨迹点加入列表
            traj.append([x, y, linear_vel, new_orientation])
    ...
    def observe_leader(self, agent, target, action, last_distance, t):
        ...
        # last_action = [self.last_action[0], self.last_action[1]]
        last_action = list(self.last_action[:8])  # 转换为列表
...
#======================= 计算奖励的=============
    def _calculate_leader_reward(self, formation_target, action, t, last_distance, last_obs_distance,traj, last_traj):#TODO
        last_distance_ = last_distance
        last_obs_distance_ = last_obs_distance
        reward_ = 0
        for i in range (0, 4):
            reward ,reward1, reward2,reward3, reward4, reward5, reward6 = 0,0,0,0,0,0,0

            vo_flag, reward1, min_dis = self._caculate_leader_vo_reward(traj[i] )

            reward2 = self._caculate_target_reward( formation_target, last_distance_ ,vo_flag, traj[i])
            reward3 = self._caculate_obstacle_reward(traj[i], last_obs_distance_)
            reward4 = self._caculate_velocity_reward(action[i*2:i*2+2], vo_flag, self.last_action[i*2:i*2+2])
            reward5 = self._caculate_side_reward(traj[i])
            ...
            last_distance_, _ = CustomEnv.calculate_relative_distance_and_angle(traj[i][:2], formation_target)
            for obs_id, obs in self.obstacles.items():
                last_obs_distance_[obs_id] = np.linalg.norm(np.array(traj[i][:2]) - np.array([obs.pos_x, obs.pos_y]))

        reward6 = self._caculate_traj_reward(traj, last_traj)
        ...
        return reward_/4 +reward6

    def _caculate_leader_vo_reward(self, traj_point):
        """traj_point:[x, y, linear_vel, orientation]"""

        vx = traj_point[2] * cos(traj_point[3])
        vy = traj_point[2] * sin(traj_point[3])
        robot_state = [traj_point[0], traj_point[1], vx, vy, self.agent_radius]
        nei_state_list = []
        obs_cir_list = []
        for obs in self.obstacles.values():
    ...
    def _caculate_target_reward(self, formation_target, last_distance, vo_flag, traj_point):
        """和编队目标之间的距离"""
        reward = 0
        dis, angle = CustomEnv.calculate_relative_distance_and_angle(traj_point[:2], formation_target)
    ...
    def _caculate_obstacle_reward(self, traj_point, last_obs_distance):
        ...
        for obs_id, obs in self.obstacles.items():
            dis, angle = CustomEnv.calculate_relative_distance_and_angle(traj_point[:2], [obs.pos_x, obs.pos_y])
            if dis <= self.obs_delta and abs(angle - traj_point[3])<=(np.pi/2)*1.2:
                vx = traj_point[2] * cos(traj_point[3])
                vy = traj_point[2] * sin(traj_point[3])
                robot_state = [traj_point[0], traj_point[1], vx, vy, self.agent_radius]
                nei_state_list = []
    ...
    def _caculate_velocity_reward(self, action, vo_flag, last_action):
        if vo_flag:
            ...
            return (action[0] + 1) *5 - abs(action[1]) *5  - abs(action[1] - last_action[1]) *15
        else:
    ...
    def _caculate_side_reward(self, traj_point):
        reward = 0
        distances = [traj_point[0], self.width - traj_point[0], traj_point[1], self.height - traj_point[1]]
    ...
    def _caculate_traj_reward(self, traj, last_traj):
        traj_ = traj[:3]
        last_traj_ = last_traj[1:4]
        position_error = []
        position_error_sum = 0
        vel_orientation_error = []
        vel_orientation_error_sum = 0
        for i in range(3):
            pos_error = np.linalg.norm(np.array(traj_[i][:2]) - np.array(last_traj_[i][:2]))
            vel_ori_err = np.linalg.norm(np.array(traj_[i][2:]) - np.array(last_traj_[i][2:]))
            position_error.append(pos_error)
            position_error_sum += pos_error
            vel_orientation_error.append(vel_ori_err)
            vel_orientation_error_sum += vel_ori_err

        return -position_error_sum*10
#======================= 计算奖励的=============
